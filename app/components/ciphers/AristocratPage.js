...
export default function AristocratPage() {
...
//   const [lettersLeft, setLettersLeft] = useState([]);
  ...
  useEffect(() => {
    const setup = async () => {
        const [quote, keyword] = await getQuote('aristocrat');
  ...
  const giveupInput = async () => {
    ...
    const quote = await giveup(quoteID, "aristocrat");
  ...
  const checkInput = async () => {
    const solveTime = Math.floor((Date.now() - startTime) / 1000);
    const [result, quote] = await submit(quoteID, "aristocrat", solveTime, answer);
  ...
  return (
    <div>
      <div id={mainStyles["main-description"]}>Solve this aristocrat cipher by {author}</div>
          ...
          <div key={wIdx} className={mainStyles["main-word-item"]}>
            {word.map((cell, idx) => {
              if (cell.type === 'input') {
                ...
                return (
                  <div key={idx + answer.length} className={mainStyles["special-letter"]}>{cell.cipher}</div>
                );
              }
            })}
          </div>
