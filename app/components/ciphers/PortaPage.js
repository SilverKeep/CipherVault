...
export default function PortaPage() {
  ...
  useEffect(() => {
    const setup = async () => {
        const [quote, keyword] = await getQuote('porta');
  ...
  const giveupInput = async () => {
    ...
    const quote = await giveup(quoteID, "porta");
  ...
  const checkInput = async () => {
    const solveTime = Math.floor((Date.now() - startTime) / 1000);
    const [result, quote] = await submit(quoteID, "porta", solveTime, answer);
  ...
  return (
    <div>
      <div id={mainStyles["main-description"]}>Solve this porta cipher with keyword {keyword} by {author}</div>
      <div id={mainStyles["main-grid"]}>
        {ciphertext.map((letter, idx) => (
          <div key={idx} className={mainStyles["main-grid-item"]}>
            ...
            <div tabIndex={0} onKeyDown={(e) => handleLetterInput(e, idx)} ref={(el) => (inputRefs.current[idx] = el)} className = {mainStyles["main-plain-letter"]}>{answer[idx] || ''}</div>
          </div>))}
      </div>
